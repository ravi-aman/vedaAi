...
export function ViewerShell({
  ...
  // If pdfUrl is provided and mime is PDF, use PdfViewer
  if (pdfUrl && mime?.includes("pdf")) {
    return <PdfViewer pdfUrl={pdfUrl} pages={pages} highlights={highlights} activePageId={activePageId} />;
  }

  // If pdfUrl is image, render image with highlights
  if (pdfUrl && mime?.startsWith("image/")) {
    return (
      <div ref={containerRef} className="flex-1 overflow-auto bg-[#E8E8E8] p-4 sm:p-6 flex flex-col items-center gap-6">
        <div className="relative bg-white shadow-md rounded-lg overflow-hidden shrink-0" style={{ width: "100%", maxWidth: 640 }}>
          ...
          <div className="absolute inset-0 pointer-events-none">
            {highlights.map((hr, idx) =>
              hr.boxes.map((box, bi) => (
                <div
                  key={`${idx}-${bi}`}
                  className="absolute border-2 rounded-sm bg-[#FF6B2C]/20 border-[#FF6B2C]"
                  style={{
                    left: `${box.x * 100}%`,
                    top: `${box.y * 100}%`,
                    width: `${box.width * 100}%`,
  ...
  // Fallback to placeholder (for old jobs without pdfUrl)
  useEffect(() => {
  ...
  if (pages.length === 0) {
    return (
      <div className="flex-1 bg-[#F0F0F0] flex items-center justify-center text-sm text-gray-500">
        No pages — answer sheet not available
      </div>
    );
  }
    ...
    <div ref={containerRef} className="flex-1 overflow-auto bg-[#E8E8E8] p-4 sm:p-6 flex flex-col items-center gap-6">
      {pages.map((page) => {
        ...
        return (
          ...
          >
            {/* Page background placeholder — shown only when pdfUrl not available */}
            <div className="absolute inset-0 bg-white flex flex-col">
              <div className="h-[6%] border-b border-gray-100 flex items-center px-4 text-[10px] text-gray-400">
                Page {page.pageNumber} • {page.width.toFixed(0)}×{page.height.toFixed(0)} {pdfUrl ? "" : "(placeholder — PDF not loaded)"}
              </div>
