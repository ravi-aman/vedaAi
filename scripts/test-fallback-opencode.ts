...
dotenv.config();
process.env.VISION_PROVIDER_ORDER = "openrouter,opencode";
...
async function main(){
  ...
  console.log(`chain ${chain.map(p=>p.id).join(",")}`);
  ...
  for(let idx=0; idx<chain.length; idx++){
    ...
    try {
      ...
      console.log(`SUCCESS ${prov.id}: ${res.pages.length} pages`);
      console.log(`fallbackUsed=${idx>0}`);
      return;
    } catch(e:any){
      const cls = classifyError(e);
      console.log(`FAILED ${prov.id} ${cls.code} ${String(e.message).slice(0,300)}`);
      if(idx < chain.length-1) console.log(`fallback to ${chain[idx+1].id}`);
    }
  }
}
