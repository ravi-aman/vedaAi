...
**Updated:** 8/30/2026, 6:07:13 PM

---

## User

# VedaAI — MULTI-PROVIDER / MULTI-MODEL VISION CONFIGURATION
# OPENROUTER + NVIDIA + OPENCODE
#
# GOAL:
# Make the entire Vision provider/model system dynamically configurable through
# .env ONLY.
#
...
# - endpoint/base URL per provider
...
# DO NOT START CODING BEFORE UNDERSTANDING HOW PROVIDERS/MODELS ARE CURRENTLY
# RESOLVED AND WHERE ANY VALUES ARE HARDCODED.
#
# DO NOT MODIFY OCR/mapping/QuestionTree/AnswerGraph architecture except where
# absolutely necessary to connect the provider abstraction.
#
# OCR remains PaddleOCR.
# Textract must remain absent.
#
# Vision providers to support:
#
# 1. NVIDIA
# 2. OpenRouter
# 3. OpenCode
#
# All three must be first-class providers.
...
docs/VISION_PROVIDER_MIGRATION_BASELINE.md
...
====================================================================
PHASE 1 — COMPLETE PROVIDER AUDIT
====================================================================

Read every production file involved in:
...
Search source for literals including:
...
3. test fixture
4. documentation
5. hardcoded production value
6. suspicious fallback
7. intentional default.

There must be NO production hardcoding after migration.

====================================================================
PHASE 2 — DEFINE A SINGLE PROVIDER CONTRACT
====================================================================

Create/retain one generic interface:
...
interface VisionProvider {
  ...
  analyzePage(input: VisionPageInput): Promise<VisionResult>

  analyzeDocument(input: VisionDocumentInput): Promise<VisionResult>

  analyzeAmbiguousMapping(
    input: VisionMappingInput
  ): Promise<VisionAdjudicationResult>

  preflight(): Promise<VisionPreflightResult>
}

Provider-specific implementation must be hidden behind this interface.

Providers:

NvidiaVisionProvider
OpenRouterVisionProvider
OpenCodeVisionProvider

No downstream code should care which provider is active.

====================================================================
PHASE 3 — NORMALIZE PROVIDER IDs
====================================================================
...
====================================================================
PHASE 4 — .ENV MUST BE THE SOURCE OF TRUTH
====================================================================

Create a clean structured .env layout.
...
VISION_PROVIDER_ORDER=nvidia,openrouter,opencode
...
Names may differ if the current project's conventions require it.

BUT:

Every configurable provider/model/base URL/key MUST come from .env.

Do not hardcode the selected model in TypeScript.

====================================================================
PHASE 5 — IMPORTANT: NO API KEYS IN GIT
====================================================================
...
Validate unknown IDs and fail configuration early.

Examples:

VISION_PROVIDER_ORDER=openrouter,nvidia,opencode

must work WITHOUT source changes.

====================================================================
PHASE 7 — PRIMARY PROVIDER
====================================================================

The first enabled provider in:
...
If the user changes .env to:

openrouter,nvidia,opencode

OpenRouter becomes first.
...
Each provider has its own configurable model:
...
If environment value is missing:

configuration error OR provider disabled,
depending on ENABLED flag.

Do not silently use a hidden model.

====================================================================
PHASE 9 — BASE URL SELECTION
====================================================================

Base URLs are also environment-driven.

Do not hardcode provider endpoints in request code if the architecture is intended
to allow replacement.

Defaults may be defined ONLY as explicit configuration defaults in ONE config
module.
...
and document which default belongs to each provider.

The implementation must not scatter URLs across provider classes.

====================================================================
PHASE 10 — PROVIDER CONFIG OBJECT
====================================================================

Create a single normalized runtime config:
...
Only config module reads process.env.

Downstream code receives typed config.

Do not access process.env directly throughout the codebase.

====================================================================
PHASE 11 — PROVIDER FACTORY
====================================================================
...
1. reads normalized config
2. chooses provider implementation
3. constructs client
4. performs preflight when requested.
...
OpenRouterVisionProvider directly.

====================================================================
PHASE 12 — PROVIDER FALLBACK CHAIN
====================================================================
...
But DO NOT fallback on every possible error automatically.
...
RATE_LIMIT
→ bounded retry/backoff, then fallback if needed

TIMEOUT
→ bounded retry/backoff, then fallback

5xx
→ bounded retry/backoff, then fallback

SCHEMA_ERROR
→ provider-specific bounded retry first
→ fallback only after retry if policy allows

Never silently hide all provider failures.

====================================================================
PHASE 13 — PROVIDER PRE-FLIGHT
====================================================================
...
Preflight should validate as much as provider API safely allows:

- API key
- endpoint connectivity
- model availability
- minimal request capability.

Do not burn expensive tokens in a large test request.

If provider is unavailable:

do not start dozens of Vision requests.

====================================================================
PHASE 14 — NVIDIA IMPLEMENTATION
====================================================================

Implement a proper NvidiaVisionProvider.

Use the provider's configured:
...
NVIDIA's hosted API is OpenAI-compatible at:

https://integrate.api.nvidia.com/v1

but keep the actual URL in configuration.

The provider adapter must normalize its response into the common VisionResult
contract.

Do not leak NVIDIA-specific response shapes downstream.

====================================================================
PHASE 15 — OPENROUTER IMPLEMENTATION
====================================================================

Keep current OpenRouter provider functionality.
...
Do not remove OpenRouter.

Do not change its behavior unnecessarily.

====================================================================
PHASE 16 — OPENCODE IMPLEMENTATION
====================================================================
...
Do not assume OpenCode uses exactly the same endpoint/protocol as the other
providers.

Audit the actual endpoint and request/response format before implementation.

Normalize it into VisionProvider.

Do not put OpenCode-specific logic into generic mapping/fusion code.

====================================================================
PHASE 17 — CAPABILITY DECLARATION
====================================================================

Every provider/model may have capabilities:
...
Use capability metadata rather than guessing.

If a configured model is not vision-capable:

preflight must fail.

Do not attempt to send images to a text-only model.

====================================================================
PHASE 18 — DYNAMIC MODEL CHANGE
====================================================================
...
must immediately change the selected model after restart.

Changing:

VISION_PROVIDER_ORDER

must change provider preference.

Changing:

VISION_GLOBAL_CONCURRENCY

must change concurrency.

Changing:

VISION_BATCH_SIZE

must change batching.

NO SOURCE CODE EDIT should be required.

====================================================================
PHASE 19 — PROVIDER LOGGING
====================================================================

Every Vision request must log:
...
====================================================================
PHASE 20 — DO NOT BREAK THE CURRENT PARALLEL PIPELINE
====================================================================

The upstream architecture MUST remain:
...
Changing providers must NOT reintroduce:

OCR → Vision

serialization.

The provider abstraction only changes WHICH external Vision service receives the
request.

====================================================================
PHASE 21 — FALLBACK MUST PRESERVE DOCUMENT STATE
====================================================================
...
the final result must clearly record:

preferredProvider = nvidia
actualProvider = openrouter
fallbackReason = ...

Do not pretend it was NVIDIA.

Per-page/batch results must record the actual provider used.

====================================================================
PHASE 22 — PROVIDER FALLBACK + CONCURRENCY
====================================================================

Important:

Do not let each parallel branch independently run an unlimited provider fallback
chain.
...
NVIDIA requests
+
OpenRouter fallback requests
+
OpenCode fallback requests

from exploding simultaneously.

====================================================================
PHASE 23 — PROVIDER-SPECIFIC RATE LIMITS
====================================================================
...
This allows provider-specific safety.

====================================================================
PHASE 24 — MODEL PREFERENCE VS PROVIDER PREFERENCE
====================================================================
...
if provider == "nvidia"
  model = "some hardcoded model"

unless the value comes from config.

====================================================================
PHASE 25 — ENV VALIDATION
====================================================================
...
Invalid configuration should fail early with:

CONFIGURATION_ERROR

Do not silently use defaults that hide configuration mistakes.

====================================================================
PHASE 26 — CLEAN .ENV
====================================================================

Reorganize .env into sections:
...
Remove stale/duplicate variables.
...
old OpenRouter settings
old Vision settings.

Do not delete a variable until code search proves it is unused.
...
====================================================================
PHASE 27 — .ENV.EXAMPLE
====================================================================

Create a clean production-safe example.
...
Use actual project defaults where appropriate.

====================================================================
PHASE 28 — CONFIG SOURCE-OF-TRUTH AUDIT
====================================================================

After implementation:

Search production code for all model IDs and provider names.

Production model IDs should ONLY appear in:
...
Provider names may appear in provider factory/adapter code obviously.

====================================================================
PHASE 29 — TEST PROVIDER SWITCHING
====================================================================
...
CASE C:
VISION_PROVIDER_ORDER=opencode,nvidia,openrouter
...
and verify runtime reports the new model.
...
====================================================================
PHASE 31 — REAL VISION SMOKE TEST
====================================================================

For every provider with valid credentials:

Use a small real page image.
...
====================================================================
PHASE 32 — REAL VISION DOCUMENT TEST
====================================================================

After smoke test:

run real question-paper pages
+
real answer-sheet pages.
...
using the selected preferred provider.

====================================================================
PHASE 33 — FALLBACK TEST
====================================================================

Force preferred provider unavailable in a controlled test.

Example:

invalid preferred key

Then verify:

preferred provider fails
fallback provider succeeds
result records actual provider.

Do not disable all providers accidentally.

====================================================================
PHASE 34 — PROVIDER FAILURE REPORT
====================================================================

Generate:

artifacts/<jobId>/vision-provider-metrics.json
...
====================================================================
PHASE 35 — PRESERVE ACCURACY
====================================================================

Changing provider configuration must not change:

QuestionTree architecture
AnswerGraph architecture
Mapping architecture
Highlight architecture

unless the new provider returns materially different evidence.

Provider abstraction should be interchangeable.

====================================================================
PHASE 36 — NO SILENT PROVIDER SWAPS
====================================================================

If fallback occurs:

show it in logs/artifacts.
...
when OpenRouter actually generated the result.

====================================================================
PHASE 37 — COST CONTROL
====================================================================

Add provider-level usage metrics.
...
Do not pretend to know exact billing unless provider gives reliable usage data.
...
- only server-side provider clients
- .env ignored by git.
...
====================================================================
PHASE 40 — FINAL PARALLEL PIPELINE CHECK

Changing provider configuration must preserve:
...
Generate timeline proving this still works.

====================================================================
PHASE 41 — FINAL REAL E2E

Use real:

27-page question paper
+
31-page handwritten answer sheet.
...
====================================================================
PHASE 42 — FINAL ACCEPTANCE

PASS only if:

[ ] current provider architecture audited
[ ] all provider/model hardcoding audited
[ ] NVIDIA provider implemented
...
[ ] enabled/disabled providers .env-driven
[ ] provider fallback implemented
[ ] fallback reason recorded
[ ] provider preflight implemented
[ ] capability checks implemented
[ ] provider-specific errors classified
[ ] global concurrency preserved
[ ] QP OCR || AS OCR preserved
[ ] QP Vision || AS Vision preserved
[ ] OCR || Vision preserved
[ ] no duplicate requests
[ ] no secrets logged
[ ] no production hardcoded model IDs
[ ] .env cleaned
[ ] .env.example cleaned
...
The source code must not know which model I currently prefer.
...
and after restarting the server, the preferred provider must change.

I should also be able to change:
...
FILES CHANGED
ENV VARIABLES ADDED/REMOVED
PROVIDER ORDER IMPLEMENTATION
MODEL SELECTION IMPLEMENTATION
FALLBACK IMPLEMENTATION
PREFLIGHT IMPLEMENTATION
CAPABILITY DETECTION
CONCURRENCY BEHAVIOR
REAL PROVIDER TEST RESULTS
...
Never claim a provider works unless a real request succeeded.

Never claim a model supports images unless capability/preflight proves it.

Never claim fallback works unless fallback has actually been tested.

Never claim the configuration is dynamic if changing .env still requires source
changes.
...
The existing four-way parallel OCR/Vision architecture must remain upstream.

Provider/model selection must be completely dynamic.

Start with forensic audit.
Then implement.
Then test.
Then verify with real providers.
Then run real 27p + 31p documents.

NO HARDcoded model IDs.
NO hardcoded provider preference.
NO fake fallback.
NO silent fallback.
NO mocks in production.
NO breaking the current parallel pipeline.


Ek important current-source fact: OpenCode Zen ka current official documentation multiple model IDs and differing API paths list karta hai, including OpenAI-compatible `/chat/completions` and OpenAI `/responses` depending on model, so **agent ko OpenCode ko generic “same endpoint as everyone” maan kar implement nahi karna chahiye**. :contentReference[oaicite:1]{index=1}

Aur NVIDIA ke current hosted API examples OpenAI-compatible `https://integrate.api.nvidia.com/v1` use karte hain, isliye us provider ke liye ek reusable OpenAI-compatible adapter practical hai. :contentReference[oaicite:2]{index=2}

**Bottom line:** tumhara `.env` ideally ek control panel ban jayega:
